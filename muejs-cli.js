...
// Github api
var GITHUB_REPO = "https://api.github.com/repos/muejs/Mue-Template/releases";
// Github下载: https://github.com/muejs/Mue-Template/archive/1.5.14.zip
// Gitee API 
var GITEE_REPO = "https://gitee.com/api/v5/repos/muejs/Mue-Template/releases";
// Gitee 下载, https://gitee.com/muejs/Mue-Template/repository/archive/1.5.14.zip

var MUE_TEMPLATE_RELEASE_URL = GITEE_REPO;
...
function toggleRepo(name) {
    ...
    switch (name) {
        case "github":
            MUE_TEMPLATE_RELEASE_URL = GITHUB_REPO;
            log("change repo github", MUE_TEMPLATE_RELEASE_URL)
            break;
        case "gitee":
            MUE_TEMPLATE_RELEASE_URL = GITEE_REPO;
            log("change repo gitee", MUE_TEMPLATE_RELEASE_URL)
            break;
    }
}

function getReleaseUrl(tag) {
    return MUE_TEMPLATE_RELEASE_URL + "/" + (tag ? `tags/${tag}` : "latest");
}
...
function getLastReleasePath() {
    ...
    // 必须跟Mue模板的github的地址目录名一致.
    const pattern = "Mue-Template";
    for (let f of files) {
 ...
 * @param {Function} cb 通过该回调返回 release 的路径，一般形如 ~/.mue-weex/template/0.1.0
 */
...
/**
 * 升级项目的mue版本,不覆盖index.html,index.js两个文件.
 * @param  {string} [name] project name.
 * @param  {string} [version] mue version.
 * @param  {string} [platformName] init platforms/ dir with specified template
...
function updateProject(names, version, platformName, devName, repoName) {
    ...
    function createProject() {
        ...
        fetchRelease(version, function(releasePath) {
            ...
            // 平台
            let cssCachePath = path.join(cachePath, "css");
            let cssFileCachePath = path.join(cssCachePath, "mue.css");
            let jsCachePath = path.join(cachePath, "js");
            let jsFileCachePath = path.join(jsCachePath, "mue.js");
            // 项目目录
            let cssPath = path.join(rootName, "css");
            let cssFilePath = path.join(cssPath, "mue.css");
            let jsPath = path.join(rootName, "js");
            let jsFilePath = path.join(jsPath, "mue.js");
            ...
            try {
                // 复制文件到项目的cache目录
                fs.copySync(releasePath, cachePath);
                // 再从cache复制mue.css文件到根目录,样式是通用的
                fs.copySync(cssFileCachePath, cssFilePath);
            } catch (e) {
                warn(`copy mue.css file error`);
            }
            ...
            if (platformName) {
                ...
                try {
                    // 再从cache复制mue.js webapp版文件到根目录
                    fs.copySync(jsFileCachePath, jsFilePath);
                } catch (e) {
                    warn(`copy mue.js file error`);
                }
            ...
            function initPlatform(platformDirName) {
                ...
                log("Update platform...");
                let muejs = path.join(platformDirName, "js");
                let muejsSrc = path.join(rootName, "js");
                // 更新平台下的js目录
                // fs.copySync(platformDirName, rootName, { filter: filterFunc });
                fs.copySync(muejs, muejsSrc);
...
function displayReleases(from) {
    ...
    from = from || "gitee";
    toggleRepo(from);
    request.get(MUE_TEMPLATE_RELEASE_URL, function(err, res, body) {
        if (err) error(err);
...
var args = yargs
    .command({
        ...
        desc: "Create a Mue project. Default to use latest version of template. try to join argument '-t' with template name to change template, join argument '-p' with platform name to change platform.",
        builder: (yargs) => {
    ...
    .command({
        ...
        desc: "update a Mue project. Default to use latest version of template. try to join argument '-p' with platform name to update platform.",
        builder: (yargs) => {
